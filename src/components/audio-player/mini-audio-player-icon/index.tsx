...
const MiniAudioPlayer = () => {
  ...
  const externalVideoStreamUrl = useSelector(
  ...
  const currTime = useSelector(
    state =>
      state.externalVideoMeetingsCollection.streamExternalVideoMeeting?.id
        ?.time,
  );
  const currEvent = useSelector(
    state => state.externalVideoMeetingsCollection.eventName,
  );
  ...
  // get the filename
  const url = new URL(
    `https://${host}${externalVideoStreamUrl}&sessionToken=${sessionToken}`,
  );
  ...
  return (
    <Styled.Container>
      <ExpandedCard
        content={<Styled.PlayIcon />}
        expandedContent={
          <Styled.Card>
            <AudioSlider
              filename={filename}
              audioSource={
                host && externalVideoStreamUrl && sessionToken && soundUri
              }
              positionFromServer={currTime}
              isPlayingFromServer={
                currEvent === 'play' || externalVideoStream?.id?.state === 1
              }
            />
          </Styled.Card>
        }
