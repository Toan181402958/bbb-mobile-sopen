...
const LKVideoControls = ({
  ...
  const unpublishCamera = useCallback(async () => {
    const publications = tracks.map(
      trackReference => trackReference.publication,
    );
    const localPublications = publications.filter(
      publication => publication?.isLocal,
    );
      ...
      await Promise.all(
        localPublications.map(publication =>
          localParticipant
            ...
            .finally(() => {
              if (publication && publication.trackName)
                sendUserStopWebcam(publication.trackName);
            }),
        ),
      );
    } catch (error) {
  ...
  const onButtonPress = useDebounce(
    ...
    }, [disabled, isActive, localCameraId, publishCamera, unpublishCamera]),
    1000,
  );
