...
import {useAudioJoin} from '../../hooks/use-audio-join.ts';
...
const LiveKitObserver = ({room, usingAudio}) => {
  ...
  const audioManagerInitialized = useSelector(
    state => state.audio.audioManagerInitialized,
  );
...
const BBBLiveKitRoom = ({children}) => {
  ...
  const sessionToken = useSelector(
    state => state.client.meetingData.sessionToken,
  );
  const isClientConnected = useSelector(
    state => state.client.sessionState.connected,
  );
  const isClientLoggedIn = useSelector(
    state => state.client.sessionState.loggedIn,
  );
  const isAudioConnected = useSelector(state => state.audio.isConnected);
  const isAudioConnecting = useSelector(
    ({audio}) => audio.isConnecting || audio.isReconnecting,
  );
  ...
  const {cameraBridge, screenShareBridge, audioBridge} =
    meetingData?.meeting[0] || {};
  const usingAudio = audioBridge === 'livekit';
  const shouldUseLiveKit =
    cameraBridge === 'livekit' || screenShareBridge === 'livekit' || usingAudio;
  ...
  const initializeMediaManagers = bridges => {
    ...
    if (bridges.cameraBridge === 'bbb-webrtc-sfu')
      VideoManager.init(mediaManagerConfigs);
    if (bridges.screenShareBridge === 'bbb-webrtc-sfu')
