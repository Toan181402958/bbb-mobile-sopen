...
const SFUVideoControls = ({
  ...
  const userRequestedHangup = useSelector(
    state => state.video.userRequestedHangup,
  );
  const localVideoStreams = useSelector(selectLocalVideoStreams);
  const ready = useSelector(
    state => isClientReady(state) && state.video.signalingTransportOpen,
  );
  const mediaServer = useSelector(state =>
    selectMetadata(state, 'media-server-video'),
  );
  const recordingAdapter = useSelector(state =>
    selectMetadata(state, 'sfu-recording-adapter'),
  );
