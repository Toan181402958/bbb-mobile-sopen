...
const useJoinMeeting = url => {
      ...
      .then(data => {
        if (data.status === 200) {
          ...
          setLocalSessionToken(
            UrlUtils.parseQueryString(data.url).sessionToken,
          );
  ...
  const payloadSizeCheckLink = new ApolloLink((operation, forward) => {
    ...
    console.log(
      `Valid ${operation.operationName} payload. Following with the query.`,
    );
  ...
  async function connectGraphqlServer() {
    ...
    try {
      const subscription = createClient({
        ...
        on: {
          error: error => {
            ...
            console.error(
              `Error: on subscription to server: ${JSON.stringify(
                error,
                null,
                2,
              )}`,
      ...
      wsLink.setOnError(error => {
        throw new Error(
          'Error: on apollo connection'.concat(JSON.stringify(error) || ''),
        );
      });
    } catch (error) {
      throw new Error(
        'Error creating WebSocketLink: '.concat(JSON.stringify(error) || ''),
      );
    }
    let client;
    try {
      ...
      throw new Error(
        'Error creating Apollo Client: '.concat(JSON.stringify(error) || ''),
      );
    }
  }
